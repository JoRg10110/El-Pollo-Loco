class Keyboard {
    ...
    bindKeyPressEvents(){
        window.addEventListener('keydown', (e) => {
            if (e.key === 'b' || e.key === 'B') this.debugMode = !this.debugMode;
        })
    ...
    bindBtnEvents() {
        setTimeout(() =>{
            ...
            if (btnUp){
                btnUp.addEventListener('touchstart', (e) => {e.preventDefault(); this.SPACE = true;});
                btnUp.addEventListener('touchend', (e) => {e.preventDefault(); this.SPACE = false;});
            }
            let btnThrow = document.getElementById('btn-Throw');
            if (btnThrow){
                btnThrow.addEventListener('touchstart', (e) => {e.preventDefault(); this.D = true;});
                btnThrow.addEventListener('touchend', (e) => {e.preventDefault(); this.D = false;});
            }
        }, 500); 
    }
}
